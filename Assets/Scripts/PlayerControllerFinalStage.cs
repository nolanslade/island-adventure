...
public class PlayerControllerFinalStage : MonoBehaviour {
	...
	void Start () {
		//currentLevel = 1;
		currentLevel = 0;
		HeartUI = GameObject.FindWithTag ("heartmanager");
	}
...
//		}
		if (HeartUI.GetComponent<HeartUIManager>().GetNumLives() < 0) {
			HeartUI.GetComponent<HeartUIManager>().EndGame();
		...
		if (countdown) {
			...
			if (elapsedTime > waitTime) {
				...
				if (!GameObject.FindGameObjectWithTag ("difficultyToggle").GetComponent<DifficultyToggle> ().Completionist ()) {
					GameObject.FindGameObjectWithTag ("difficultyToggle").GetComponent<DifficultyToggle> ().resetCompletionist ();
	...
	void OnTriggerEnter (Collider col) {
		...
		if (col.gameObject.tag.Equals("level_one_gate_lock")) {
			if (!hasKey) {
				// Error sound
				col.gameObject.GetComponents<AudioSource> ()[0].Play ();
			} else {
